...
public class FacilityDashboardUtil {
	...
	public static Long getHivPositiveNotLinked(String startDate, String endDate) {
		...
		String hivPositiveNotLinkedQuery = "SELECT COUNT(a.patient_id) number_hiv_positive\n" +
			...
			"       WHERE av.visit_date BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE\n" +
			...
			"       WHERE d.visit_date BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE\n" +
			...
			"       WHERE p.visit_date BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE\n" +
			...
			"           AND t.voided = 0\n" +
			"           AND t.visit_date BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE)) a\n" +
			"         LEFT JOIN\n" +
			"     (SELECT l.patient_id, l.ccc_number\n" +
			"      FROM kenyaemr_etl.etl_hts_referral_and_linkage l\n" +
			"      WHERE date(l.visit_date) BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE\n" +
			"      GROUP BY l.patient_id) l ON a.patient_id = l.patient_id\n" +
			"         LEFT JOIN (SELECT e.patient_id\n" +
			"                    FROM kenyaemr_etl.etl_hiv_enrollment e\n" +
			"                    WHERE date(e.visit_date) BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE) e\n" +
	...
	public static Long getPregnantOrPostpartumClients(String startDate, String endDate) {
		...
		String pregnantOrPostpartumQuery = "SELECT COUNT(s.patient_id) high_risk_preg_postpartum\n" +
				...
				"                     HAVING hts_date BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE\n" +
				...
				"  AND s.visit_date BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE;";
	...
	public static Long getPregnantPostpartumNotInPrep(String startDate, String endDate) {
		...
		String pregnantPostPartumNotPrepLinkedQuery = "SELECT COUNT(a.patient_id) high_risk_not_on_PrEP\n" +
			...
			"         AND s.visit_date BETWEEN DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY) AND CURRENT_DATE) a\n" +
			"         left join (select e.patient_id,\n" +
	...
	public static Long getEligibleForVl(String startDate, String endDate) {
		...
		String eligibleForVlQuery = "select count(b.patient_id) as eligible_for_vl\n" +
				...
				"                     where (TIMESTAMPDIFF(MONTH, v.date_started_art, DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY)) >= 3 and\n" +
				"                            v.base_viral_load_test_result is null)                              -- First VL new on ART\n" +
				"                        OR ((v.pregnancy_status = 1065 or v.breastfeeding_status = 1065) and\n" +
				"                            TIMESTAMPDIFF(MONTH, v.date_started_art, DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY)) >= 3 and\n" +
				"                            (v.vl_result is not null and\n" +
				"                             v.date_test_requested < v.latest_hiv_followup_visit))              -- immediate for PG & BF\n" +
				"                        OR (v.vl_result >= 200 AND\n" +
				"                            TIMESTAMPDIFF(MONTH, v.date_test_requested, DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY)) >=\n" +
				"                            3)                                                                  -- Unsuppressed VL\n" +
				"                        OR (v.vl_result < 200 AND\n" +
				"                            TIMESTAMPDIFF(MONTH, v.date_test_requested, DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY)) >= 6 and\n" +
				...
				"                        OR ((v.pregnancy_status = 1065 or v.breastfeeding_status = 1065) and\n" +
				"                            TIMESTAMPDIFF(MONTH, v.date_started_art, DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY)) >= 3\n" +
	...
	public static Long getHeiSixToEightWeeksOld(String startDate, String endDate){
		...
		String getHeiSixToEightWeeksOldQuery = "SELECT COUNT(e.patient_id)\n" +
				...
				"WHERE TIMESTAMPDIFF(WEEK, d.DOB, DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY)) BETWEEN 6 AND 8;";
	...
	public static Long getHei24MonthsOld(String startDate, String endDate) {
		...
		String hei24MonthsOldQuery = "SELECT COUNT(e.patient_id)\n" +
				...
				"WHERE TIMESTAMPDIFF(MONTH, d.dob, DATE_SUB(CURRENT_DATE, INTERVAL " + days + " DAY)) = 24;";
		try {
			Context.addProxyPrivilege(PrivilegeConstants.SQL_LEVEL_ACCESS);
