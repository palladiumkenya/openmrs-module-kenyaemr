...
public class SariCohortDefinitionEvaluator implements EncounterQueryEvaluator {
    ...
    public EncounterQueryResult evaluate(EncounterQuery definition, EvaluationContext context) throws EvaluationException {
        ...
        String qry = "SELECT encounter_id from kenyaemr_etl.etl_patient_triage t inner join person p on p.person_id=t.patient_id and p.voided=0 where t.voided = 0 and date(t.visit_date) BETWEEN date(:startDate) AND date(:endDate) ; ";
