...
private String getFcdrrDrugSummary(int drugId, int factor,int unit) {
		String query =	"SELECT \n" +
				"    IFNULL(sspu.factor, 0) AS unit_pack_size,\n" +
				...
				"INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"    ON sspu.stock_item_id = stit.stock_item_id\n" +
				"LEFT JOIN stockmgmt_stock_operation ssto_main \n" +
				"    ON stit.stock_operation_id = ssto_main.operation_type_id \n" +
				...
				"    FROM stockmgmt_stock_item_transaction stit\n" +
				"    INNER JOIN stockmgmt_stock_item ssi \n" +
				"        ON ssi.stock_item_id = stit.stock_item_id\n" +
				"    INNER JOIN stockmgmt_stock_operation ssto_receipts \n" +
				"        ON stit.stock_operation_id = ssto_receipts.stock_operation_id\n" +
				"    INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"        ON sspu.stock_item_id = stit.stock_item_id\n" +
				"    WHERE ssi.drug_id = %d\n" +
				"        AND sspu.factor = %d\n" +
				"    AND ssi.dispensing_unit_id = %d\n" +
				"        AND ssto_receipts.status = 'COMPLETED'\n" +
				"        AND stit.date_created BETWEEN DATE_SUB(DATE(:startDate), INTERVAL 1 MONTH) AND DATE_SUB(DATE(:endDate), INTERVAL 1 MONTH)\n" +
				...
				"    INNER JOIN stockmgmt_stock_item ssi \n" +
				"        ON ssi.stock_item_id = stit.stock_item_id\n" +
				"    INNER JOIN stockmgmt_stock_operation ssto_disposals \n" +
				"        ON stit.stock_operation_id = ssto_disposals.stock_operation_id\n" +
				"    INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"        ON sspu.stock_item_id = stit.stock_item_id\n" +
				"    WHERE ssi.drug_id = %d\n" +
				"        AND sspu.factor = %d\n" +
				"    AND ssi.dispensing_unit_id = %d\n" +
				"        AND ssto_disposals.status = 'COMPLETED'\n" +
				"        AND stit.date_created BETWEEN DATE_SUB(DATE(:startDate), INTERVAL 1 MONTH) AND DATE_SUB(DATE(:endDate), INTERVAL 1 MONTH)\n" +
				...
				"FROM stockmgmt_stock_item_transaction stit\n" +
				"INNER JOIN stockmgmt_stock_item ssi ON stit.stock_item_id = ssi.stock_item_id\n" +
				...
				"    INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"        ON sspu.stock_item_id = stit.stock_item_id\n" +
				"    LEFT JOIN stockmgmt_stock_operation ssto_losses \n" +
				"        ON stit.stock_operation_id = ssto_losses.stock_operation_id\n" +
				"    WHERE ssi.drug_id = %d\n" +
				"        AND sspu.factor = %d\n" +
				"    AND ssi.dispensing_unit_id = %d\n" +
				"        AND ssto_losses.status = 'COMPLETED'\n" +
				"        AND stit.date_created BETWEEN :startDate AND :endDate\n" +
				...
				"    INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"        ON sspu.stock_item_id = stit.stock_item_id\n" +
				"    LEFT JOIN stockmgmt_stock_operation ssto_pos_adj \n" +
				"        ON stit.stock_operation_id = ssto_pos_adj.stock_operation_id\n" +
				"    WHERE ssi.drug_id = %d\n" +
				"        AND sspu.factor = %d\n" +
				"    AND ssi.dispensing_unit_id = %d\n" +
				"        AND ssto_pos_adj.status = 'COMPLETED'\n" +
				"        AND stit.date_created BETWEEN :startDate AND :endDate\n" +
				...
				"    INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"        ON sspu.stock_item_id = stit.stock_item_id\n" +
				"    LEFT JOIN stockmgmt_stock_operation ssto_neg_adj \n" +
				"        ON stit.stock_operation_id = ssto_neg_adj.stock_operation_id\n" +
				"    WHERE ssi.drug_id = %d\n" +
				"        AND sspu.factor = %d\n" +
				"    AND ssi.dispensing_unit_id = %d\n" +
				"        AND stit.quantity > 0\n" +
				"        AND ssto_neg_adj.status = 'COMPLETED'\n" +
				"        AND stit.date_created BETWEEN :startDate AND :endDate\n" +
				...
				"    INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"        ON sspu.stock_item_id = stit.stock_item_id\n" +
				"    LEFT JOIN stockmgmt_stock_operation ssto_stock_take \n" +
				"        ON stit.stock_operation_id = ssto_stock_take.stock_operation_id\n" +
				"    LEFT JOIN stockmgmt_stock_operation_item ssoi \n" +
				"        ON ssoi.stock_operation_id = stit.stock_operation_id\n" +
				"    WHERE ssi.drug_id = %d\n" +
				"        AND sspu.factor = %d\n" +
				"    AND ssi.dispensing_unit_id = %d\n" +
				"        AND ssto_stock_take.status = 'COMPLETED'\n" +
				"        AND stit.date_created BETWEEN :startDate AND :endDate\n" +
				...
				"    INNER JOIN stockmgmt_stock_item ssi \n" +
				"        ON ssi.stock_item_id = stit.stock_item_id\n" +
				"    INNER JOIN stockmgmt_stock_batch ssbt \n" +
				"        ON ssbt.stock_item_id = stit.stock_item_id\n" +
				"    INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"        ON sspu.stock_item_id = stit.stock_item_id\n" +
				"    WHERE ssi.drug_id = %d\n" +
				"        AND sspu.factor = %d\n" +
				...
				"    INNER JOIN stockmgmt_stock_item_packaging_uom sspu \n" +
				"        ON sspu.stock_item_id = stit.stock_item_id\n" +
				"    LEFT JOIN stockmgmt_stock_operation ssto_requisitions \n" +
				"        ON stit.stock_operation_id = ssto_requisitions.stock_operation_id\n" +
				"    WHERE ssi.drug_id = %d\n" +
				"        AND sspu.factor = %d\n" +
				"    AND ssi.dispensing_unit_id = %d\n" +
				"        AND ssto_requisitions.status = 'COMPLETED'\n" +
				"        AND stit.date_created BETWEEN :startDate AND :endDate\n" +
				"        AND ssto_requisitions.operation_type_id IN (7)\n" +
				"    GROUP BY stit.stock_item_id\n" +
				") cur_req ON cur_req.stock_item_id = stit.stock_item_id;";

	return String.format(query, drugId, drugId, factor,unit, drugId, factor,unit, drugId, factor,unit, drugId, factor,unit, drugId, factor,unit, drugId, factor,unit, drugId, factor,unit,
			drugId, factor,unit, drugId, factor,unit, drugId, factor,unit, drugId, factor,unit);
	}
}
