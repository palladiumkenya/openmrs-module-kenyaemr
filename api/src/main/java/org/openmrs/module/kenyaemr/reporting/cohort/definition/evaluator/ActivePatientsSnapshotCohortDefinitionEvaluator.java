...
public class ActivePatientsSnapshotCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
    ...
    public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) throws EvaluationException {
		...
		String qry="select t.patient_id\n" +
				...
				"           greatest(max(e.visit_date), ifnull(max(date(e.transfer_in_date)),null)) as latest_enrolment_date,\n" +
				"           greatest(max(fup.visit_date), ifnull(max(d.visit_date),null)) as latest_vis_date,\n" +
				"           greatest(mid(max(concat(fup.visit_date,fup.next_appointment_date)),11), ifnull(max(d.visit_date),null)) as latest_tca,\n" +
