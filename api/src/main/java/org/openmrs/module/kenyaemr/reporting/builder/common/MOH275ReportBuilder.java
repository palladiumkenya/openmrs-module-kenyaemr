...
import org.openmrs.module.kenyaemr.reporting.cohort.definition.MOH275RegisterCohortDefinition;
import org.openmrs.module.kenyaemr.reporting.data.converter.CalculationResultConverter;
import org.openmrs.module.kenyaemr.reporting.data.converter.definition.occupation.*;
...
@Component
@Builds({"kenyaemr.ehrReports.report.moh275"})
public class MOH275ReportBuilder extends AbstractReportBuilder {
    ...
    protected DataSetDefinition datasetColumns() {
        ...
        dsd.setName("MOH275");
        dsd.setDescription("Occupational Therapy Register ");
		...
		AmountDataDefinition amountDataDefinition = new AmountDataDefinition();
		amountDataDefinition.addParameter(new Parameter("endDate", "End Date", Date.class));
		amountDataDefinition.addParameter(new Parameter("startDate", "Start Date", Date.class));

        ReceiptDataDefinition receiptDataDefinition = new ReceiptDataDefinition();
        receiptDataDefinition.addParameter(new Parameter("endDate", "End Date", Date.class));
        receiptDataDefinition.addParameter(new Parameter("startDate", "Start Date", Date.class));

        PaymentMethodDataDefinition paymentMethodDataDefinition = new PaymentMethodDataDefinition();
        paymentMethodDataDefinition.addParameter(new Parameter("endDate", "End Date", Date.class));
        paymentMethodDataDefinition.addParameter(new Parameter("startDate", "Start Date", Date.class));

        AssistiveTechnologyDataDefinition assistiveTechnologyDataDefinition = new AssistiveTechnologyDataDefinition();
        ...
        OTDataDefinition otDataDefinition = new OTDataDefinition();
        otDataDefinition.addParameter(new Parameter("endDate", "End Date", Date.class));
        otDataDefinition.addParameter(new Parameter("startDate", "Start Date", Date.class));
        ...
        OccupationVisitTypeDataDefinition visitTypeDataDefinition = new OccupationVisitTypeDataDefinition();
        ...
        dsd.addColumn("OT interventions", otDataDefinition, paramMapping);
		dsd.addColumn("Payment Method", paymentMethodDataDefinition, paramMapping);
        dsd.addColumn("Amount Paid", amountDataDefinition, paramMapping);
        dsd.addColumn("Receipt Number", receiptDataDefinition, paramMapping);

		MOH275RegisterCohortDefinition cd = new MOH275RegisterCohortDefinition();
