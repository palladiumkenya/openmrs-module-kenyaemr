...
public class DailyBedReturnRegisterCohortDefinitionEvaluator implements EncounterQueryEvaluator {
	...
	public EncounterQueryResult evaluate(EncounterQuery definition, EvaluationContext context) throws EvaluationException {
		...
		String qry = "SELECT ce.encounter_id from bed_patient_assignment_map ce\n" +
				"inner join kenyaemr_etl.etl_patient_demographics p on p.patient_id = ce.patient_id and  p.voided = 0 \n" +
				"where date(ce.date_started) BETWEEN date(:startDate) AND date(:endDate)and ce.voided = 0;";
